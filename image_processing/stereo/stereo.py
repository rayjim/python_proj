# -*- coding: utf-8 -*-
"""
Created on Mon Apr 14 20:09:23 2014
...
def plane_sweep_ncc(im_l,im_r,start,steps,wid):
      ...
      for displ in range(steps):
        ...
        filters.uniform_filter(norm_r*norm_r,wid,s_r) # sum denominator # store ncc scores
        filters.uniform_filter(norm_r*norm_r,wid,s_r) # sum denominator # store ncc scores
        dmaps[:,:,displ] = s/sqrt(s_l*s_r) # pick best depth for each pixel
      return argmax(dmaps,axis=2)
      
def plane_sweep_gauss(im_l,im_r,start,steps,wid):
    """ Find disparity image using normalized cross-correlation with 
    Gaussian weighted neigborhoods"""
    m,n = im_l.shape
    # arrays to hold the different sums
    mean_l = zeros((m,n))
    mean_r = zeros((m,n))
